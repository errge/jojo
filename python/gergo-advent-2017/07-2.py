input = """
mypyemv (1058) -> tdssotr, pebnvks, zaulju
snslv (48)
aytbgf (30)
mkcrsy (42)
shwcd (42)
...
class Program:
    def __init__(self, name, weight, others):
        self.name = name
        self.weight = weight
        self.others = others
    ...
    def computeweight(self):
        ...
        sum = 0
        for prg in self.others:
            sum += programs[prg].computeweight()

        self.totalweight = sum + self.weight
        return self.totalweight

    def correct(self):
        if self.others == []: return True
        othersweight = (self.totalweight - self.weight) // len(self.others)
        for prg in self.others:
            if programs[prg].totalweight != othersweight:
                return False
        return True
...
def main():
    for line in input.splitlines():
        if line == '': continue
        if line.find(' -> ') != -1:
            [prg, others] = line.split(' -> ')
            others = others.split(', ')
        else:
            prg = line
            others = []
        name, weight = prg.split(' (')
        programs[name] = Program(name, int(weight[:-1]), others)

    root = None
    for name in programs.keys():
        mutattakram = False
        for otherprg in programs.values():
            if name in otherprg.others:
                mutattakram = True
        if not mutattakram:
            root = name
    ...
    programs[root].computeweight()
    for prg in programs.keys():
        if not programs[prg].correct():
            print(f'{prg} is incorrect, weight: {programs[prg].weight}')
            for o in programs[prg].others:
                print(f'{o} weight: {programs[o].weight}, totalweight: {programs[o].totalweight}')
