...
class Processor:
    def __init__(self, input):
        ...
        self.prg = input.strip().splitlines()

    def step(self):
        target, cmd, value, _if, verifyleft, operand, verifyright = self.prg.pop(0).split()
        verifyleft = self.reg[verifyleft]
        verifyright = int(verifyright)
        verified = None
        match operand:
            case '<': verified = verifyleft < verifyright
            case '<=': verified = verifyleft <= verifyright
            case '>': verified = verifyleft > verifyright
            case '>=': verified = verifyleft >= verifyright
            case '==': verified = verifyleft == verifyright
        ...
        if verified:
            if cmd == 'inc':
                self.reg[target] += int(value)
            else:
                self.reg[target] -= int(value)

proc = Processor(input)
held = []
while proc.prg:
    proc.step()
    held.append(max(proc.reg.values()))
print(max(held))
